...
bool check_gl_extension(char* extension) {
    int count = 0;
...
int query_NPOT_capability( void )
{
	...
	if( has_NPOT_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		if(check_gl_extension("GL_ARB_texture_non_power_of_two" ) == false)
		{
...
int query_tex_rectangle_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
		if(check_gl_extension("GL_ARB_texture_rectangle" ) == false
           &&
           check_gl_extension("GL_EXT_texture_rectangle" ) == false
           &&
           check_gl_extension("GL_NV_texture_rectangle" ) == false
			)
		{
			/*	not there, flag the failure	*/
...
int query_cubemap_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
        if(check_gl_extension("GL_ARB_texture_cube_map") == false
           &&
           check_gl_extension("GL_EXT_texture_cube_map" ) == false
			)
		{
			/*	not there, flag the failure	*/
...
int query_DXT_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
		if( check_gl_extension("GL_EXT_texture_compression_s3tc") == false )
		{
			/*	not there, flag the failure	*/
